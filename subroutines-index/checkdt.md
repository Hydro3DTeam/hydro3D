...
\
**Purpose:**\
****This subroutine is used at the beginning of each new time step iteration to determine the size of the next time step based on the results of the previous iteration.\
****\
...
!##########################################################################
	...
	double precision :: Cu,Cv,Cw
     ...
     & 2.0*dom(ib)%vis(i,j,k)*(1.0/dxx+1.0/dyy+1.0/dzz)+small)
		       ...
		       do n=1,nscalar
                       if(SENERGY(n)) then
                       dtthr=0.5*Re*Pr(n)/(1.0/dxx + 1.0/dyy + 1.0/dzz)
                       dtvisc=min(dtvisc,dtthr)
		       else
                       dtthr=0.5*Re*Sc(n)/(1.0/dxx + 1.0/dyy + 1.0/dzz)
                       dtvisc=min(dtvisc,dtthr)
                       end if
		       end do !n
                    end do
