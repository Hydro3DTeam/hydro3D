...
**File:** diffusion_.for_\
__\
__**Subroutine Called in:** __ \
_flosol (flosol.for)_\
__\
__**Purpose:**\
****This subroutine approximation the diffusion term of the Navier-Stokes equation for the SIP solver using numerical method.\
\
...
! ------ Compute coefficients for diffusion terms
     ...
     & dom(ib)%su(i,j,k)+forcn*dt
		 ...
		 do n=1,nscalar
		   if(SENERGY(n)) then
                     dom(ib)%su(i,j,k)=
     & dom(ib)%su(i,j,k)+dt*grx*(1.d0-0.5*beta(n)*
     & (dom(ib)%S(i+1,j,k,n)+dom(ib)%S(i,j,k,n)))
		   end if
		 end do
		 end if
     ...
     &dom(ib)%vstar(i,j,k))+dom(ib)%su(i,j,k)
		 ...
		 do n=1,nscalar
		   if(SENERGY(n)) then
		     dom(ib)%su(i,j,k)=
     & dom(ib)%su(i,j,k)+dt*gry*(1.d0-0.5*beta(n)*
     & (dom(ib)%S(i,j+1,k,n)+dom(ib)%S(i,j,k,n)))
		   end if
		 end do
		 end if
     ...
     &dom(ib)%wstar(i,j,k))+dom(ib)%su(i,j,k)
		 ...
		 do n=1,nscalar
		   if(SENERGY(n)) then
                     dom(ib)%su(i,j,k)=
     & dom(ib)%su(i,j,k)+dt*grz*(1.d0-0.5*beta(n)*
     & (dom(ib)%S(i,j,k+1,n)+dom(ib)%S(i,j,k,n)) )
		   end if
		 end do
		 end if
