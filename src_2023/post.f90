...
!#######################################################################
      ...
      CHARACTER(25) :: gf
        ...
        write (88,*)'variables=x,y,z,u,v,w,T,Tm,Ttm'
...
!          write(95,89)'zone i=',ntoti,', j=',ntotj,', k=',ntotk,' f=point'
	   write(95,*) toti,totj,totk
	   write(95,*) pl
	   write(95,*) 
          IF(LENERGY) THEN
            do k=1,totk ; do j=1,totj ; do i=1,toti
                  write (95,88) ctime,dom(ib)%x(i),dom(ib)%y(j),dom(ib)%z(k) & 
       &                       ,dom(ib)%u(i,j,k),dom(ib)%v(i,j,k),dom(ib)%w(i,j,k),dom(ib)%p(i,j,k) &
       &                       ,dom(ib)%T(i,j,k)
