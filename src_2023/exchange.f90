!#######################################################################
! SUBROUTINES: - exchange
!              - exchange_smlvl
!#######################################################################
      SUBROUTINE  exchange(op)
!-----------------------------------------------------------------------
!     This subroutine is the menu to select the corresponding exchange
!     script for each field variables when the Local Mesh Refinement is
!     turn on. Leading to a possibility of exchanging ghost-cells which
!     do not have the same LMR.
!
!     PAPER: Cevheri,M., McSherry,R., & Stoesser,T. (2016). A local mesh
!     refinement approach for large‐eddy simulations of turbulent flows.
...
!-----------------------------------------------------------------------
!     Exchange all the field variables between the ghost-cells of
!     each subdomain and their neighbours which have the same Local Mesh
!     Refinement level.
!     NOTE 1: smlvl = same level of LMR
!     NOTE 2: called in the subroutine exchange
!#######################################################################
      use multidata
      use mpi
      use vars
