...
! Resolve the convection and diffusion components
      ...
      IF(conv_sch.eq.3 .or. conv_sch.eq.4) THEN
        do kutta=1,kuttacond
          ...
          ! Convection based on differencing scheme
          ...
          ! Diffusion = 1.Impli Euler .or. 2.Crank-Nicholson
          else
            call diffusion
          end if
      ...
      ! Convection = 1.Exp_Euler .or. 2.Adams-Bashford
      else
        call convection
        call diffusion
      end if
...
!Lagrangian Particle Tracking final checks
	   ...
	   if((mod(itime,tsteps_pt).eq.0).and.(itime.gt.itime_start)) then 			
 	           	if (myrank.eq.0) call TECPARTICLE(itime)
      	     	       call TECPLOT(itime)
!			cnt_pt = cnt_pt + 1
	   end if
        end if
