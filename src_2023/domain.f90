...
!#######################################################################
	...
	if (myrank.eq.0) then
        ...
        if (num_domains .gt. 9999) then
           print*, '=====ERROR====='
           print*, 'number of domains are exceeding the limit in exchange subroutine'
           stop
        end if
...
!#######################################################################
      ...
      call MPI_ALLREDUCE(buffer_rdiv,maxrdiv,1,MPI_INTEGER,MPI_MAX,MPI_COMM_WORLD,ierr)
      ...
      do ib=1,nbp
        if(rdiv(dom_id(ib)).gt.1) then
          dom(ib)%ngrid=ngrid_input+int(log(real(rdiv(dom_id(ib))))/log(2.0))
        end if
      ...
      call MPI_ALLREDUCE(b_rv1,b_rv2,num_domains*ngrd_gl,MPI_INTEGER,MPI_SUM,MPI_COMM_WORLD,ierr)
      ...
      DO ib=1,nbp
        ...
        if (abs(n1-nint(n1)).gt.1e-8) then
           print*,'error1 in localparameters',dom_id(ib),(dom(ib)%xel-dom(ib)%xsl),g_dx,n1
           stop
        end if
        if (abs(n2-nint(n2)).gt.1e-8) then
           print*,'error2 in localparameters',dom_id(ib),(dom(ib)%yel-dom(ib)%ysl),g_dy,n2
           stop
        end if
        if (abs(n3-nint(n3)).gt.1e-8) then
          print*,'error3 in localparameters',dom_id(ib),(dom(ib)%zel-dom(ib)%zsl),g_dz,n3
          stop
        end if
        ...
        if(dom(ib)%inext.lt.0 .and. dom(ib)%bc_east.ne.5) dom(ib)%ieu=dom(ib)%ieu-1
        if(dom(ib)%jnext.lt.0 .and. dom(ib)%bc_north.ne.5) dom(ib)%jev=dom(ib)%jev-1
        if(dom(ib)%knext.lt.0 .and. dom(ib)%bc_top.ne.5) dom(ib)%kew=dom(ib)%kew-1
        ...
        if(solver.eq.2) then
          ...
          if(mod(nicell,pow2).ne.0 .or. mod(njcell,pow2).ne.0 .or. mod(nkcell,pow2).ne.0) then
          ...
          do glevel=1,ngrd_gl-1
            ...
            dom(ib)%cntp(glevel+1)=dom(ib)%cntp(glevel)+mgc_i*mgc_j*mgc_k
          end do
          ...
          do glevel=1,ngrd_gl
            ...
            mgc_j=(dom(ib)%jep-dom(ib)%jsp+1)/2**(gl-1)+2
            mgc_k=(dom(ib)%kep-dom(ib)%ksp+1)/2**(gl-1)+2
            dom(ib)%nwork=dom(ib)%nwork+mgc_i*mgc_j*mgc_k+8*(mgc_i-2)*(mgc_j-2)*(mgc_k-2)
	   ...
	   allocate (dom(ib)%cof(dom(ib)%nwork))
        ...
        write (6,88) 'mydom#:',dom_id(ib),' i:',dom(ib)%ttc_i,' j:',dom(ib)%ttc_j,' k:',dom(ib)%ttc_k
      ...
      buffer_nemax = nemax
      call MPI_ALLREDUCE(buffer_nemax,nemax,1,MPI_INTEGER,MPI_MAX,MPI_COMM_WORLD,ierr)

      buffer_nmax = nmax
      call MPI_ALLREDUCE(buffer_nmax,nmax,1,MPI_INTEGER,MPI_MAX,MPI_COMM_WORLD,ierr)
