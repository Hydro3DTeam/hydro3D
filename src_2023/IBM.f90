...
!-----------------------------------------------------------------------
!     Read in_geom.cin and initialise the main arrays to perform
!     the Immersed Boundary Method. It also create the
!#######################################################################
      use vars
      use multidata
      use imb
...
!Distribute properties of the actuator line turbines
	...
	if(itime.eq.itime_start) CALL actuatorline_initial!(K)			!Turbine via Actuator Line model
