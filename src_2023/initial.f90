!#######################################################################
! SUBROUNTINES: - read_control
!               - initial
!               - initflowfield
!#######################################################################
      SUBROUTINE read_control
!-----------------------------------------------------------------------
!     Read control.cin, master file that specifies the numerical method the use for 
!     the N-S equation, the time and coarse mesh of the simulation. It also 
!     turn on/off logical to use the different modules: (IBM,LSM,Scalar,LPT)
!#######################################################################
      use vars
      use multidata
      use mpi
...
!------DOMAIN SIZE AND DISCRETIZATION -------------------------------------
	 ...
	 read (12,*) L_n,fric
	  ...
	  read (12,*)
	 read (12,*) n_unstpt
	 allocate(id_unst(n_unstpt),i_unst(n_unstpt),j_unst(n_unstpt),k_unst(n_unstpt))
	 do i=1,n_unstpt
      ...
      if (LLSM) call read_lsm_input_data				      

      if (LSCALAR .or. LENERGY) call read_energy_input_data	

      nswp(1)=5 ; nswp(2)=5 ; nswp(3)=5 ; nswp(4)=20

!Pressure forcing for periodic conditions: PABLO 02/2018
      PERIODIC=.false. 
      pressureforce=.false. ; pressureforce_y=.false. ; pressureforce_z=.false.
      ...
      if(differencing.eq.3 .and. pl_ex.ne.2) then
        pl_ex=2
        if(myrank.eq.0) then
          write(6,*) 'error: you select WENO but do not assign', &
     &'   correct number of ghost planes,now it is corrected to 2'
        ...
        if(SGS .and. sgs_model.eq.3 .and. pl_ex.ne.2) then
          pl_ex=2
	   if(myrank.eq.0) then
            write(6,*) 'error: you select 1-EQN model but do not assign', &
     &'     correct number of ghost planes,now it is corrected to 2'
	   endif
        end if
...
!#######################################################################
      SUBROUTINE initial
!-----------------------------------------------------------------------
!     Initialise the subdomain type data stucture [dom(:)%].
!     This structure defines variables that applies to every Eulerian 
!     subdomain.
!#######################################################################
      use vars
      use mpi
      ...
      do ib=1,nbp
        ...
        ! MPI tag that performs the parrallel transfer between the Ghost Cell:
        do i=1,26
        ...
        ! Cell center of the cells:
        allocate(dom(ib)%xc(tti),dom(ib)%yc(ttj),dom(ib)%zc(ttk))

        ! Instanteneous free divergence velocity field:
        allocate(dom(ib)%ustar(tti,ttj,ttk),dom(ib)%vstar(tti,ttj,ttk),dom(ib)%wstar(tti,ttj,ttk))
        
        ! Instantaneous velocity field:
        allocate(dom(ib)%u(tti,ttj,ttk),dom(ib)%v(tti,ttj,ttk),dom(ib)%w(tti,ttj,ttk))

        ! Previous time step (itime-1) velocity field:
        allocate(dom(ib)%uo(tti,ttj,ttk),dom(ib)%vo(tti,ttj,ttk),dom(ib)%wo(tti,ttj,ttk))
        
        ! Previous time step (itime-1) velocity variance:
        allocate(dom(ib)%uoo(tti,ttj,ttk),dom(ib)%voo(tti,ttj,ttk),dom(ib)%woo(tti,ttj,ttk))
        
        ! Mean velocity and pressure field:
        allocate(dom(ib)%um(tti,ttj,ttk),dom(ib)%vm(tti,ttj,ttk),dom(ib)%wm(tti,ttj,ttk))
        allocate(dom(ib)%pm(tti,ttj,ttk))

        ! Mean Reynold shear stress:
        allocate(dom(ib)%uvm(tti,ttj,ttk),dom(ib)%uwm(tti,ttj,ttk),dom(ib)%vwm(tti,ttj,ttk))

        ! Mean Velocity and Pressure Variance:
        allocate(dom(ib)%uum(tti,ttj,ttk),dom(ib)%vvm(tti,ttj,ttk),dom(ib)%wwm(tti,ttj,ttk))
        ...
        ! Number of time step for the first and second order statistic averaging:
	 ...
	 dom(ib)%ntav1=0 ; dom(ib)%ntav2=0
        ...
        ! Kinematic Instantaneous and Average viscosity:
        allocate(dom(ib)%vis(tti,ttj,ttk), dom(ib)%vism(tti,ttj,ttk))

        allocate(dom(ib)%ksgs(tti,ttj,ttk),dom(ib)%ksgso(tti,ttj,ttk))

        allocate(dom(ib)%eps(tti,ttj,ttk),dom(ib)%epsm(tti,ttj,ttk),dom(ib)%epso(tti,ttj,ttk))
	 ...
	 if(LSCALAR) then
	   ...
	   allocate(dom(ib)%ibfactor(tti,ttj,ttk))
        ...
        if(L_LSM .or. L_LSMbase)  then
          ! Flow density and dynamic viscosity:
          allocate(dom(ib)%dens(tti,ttj,ttk),dom(ib)%mu(tti,ttj,ttk))

          ! Level-Set-Method Instantaneous and Mean Free-Surface:
	   allocate(dom(ib)%phi(tti,ttj,ttk),dom(ib)%phim(tti,ttj,ttk))
        ...
        if (L_LSM) then! .or. L_LSMbase) then
          dom(ib)%ijkp_lsm = 0
          dom(ib)%ijkp_lsm(1)=(dom(ib)%ttc_i-2*pl)*(dom(ib)%ttc_j-2*pl)*(dom(ib)%ttc_k-2*pl)
          do glevel=2,dom(ib)%ngrid
            ...
            dom(ib)%ijkp_lsm(glevel)=dom(ib)%ijkp_lsm(glevel-1)+(mgc_i-2)*(mgc_j-2)*(mgc_k-2)
          end do
        ...
        if (differencing.eq.3) then 
          allocate(dom(ib)%d1(tti,ttj,ttk))
          allocate(dom(ib)%dphi_dxplus(tti,ttj,ttk),dom(ib)%dphi_dxminus(tti,ttj,ttk))
          allocate(dom(ib)%dphi_dyplus(tti,ttj,ttk),dom(ib)%dphi_dyminus(tti,ttj,ttk))
          allocate(dom(ib)%dphi_dzplus(tti,ttj,ttk),dom(ib)%dphi_dzminus(tti,ttj,ttk))
        end if
        
        ! Wall Function:
        allocate(dom(ib)%tauwe(dom(ib)%jsp-1:dom(ib)%jep+1,dom(ib)%ksp-1:dom(ib)%kep+1))
        allocate(dom(ib)%tauww(dom(ib)%jsp-1:dom(ib)%jep+1,dom(ib)%ksp-1:dom(ib)%kep+1))
        allocate(dom(ib)%tauwn(dom(ib)%isp-1:dom(ib)%iep+1,dom(ib)%ksp-1:dom(ib)%kep+1))
        allocate(dom(ib)%tauws(dom(ib)%isp-1:dom(ib)%iep+1,dom(ib)%ksp-1:dom(ib)%kep+1))
        allocate(dom(ib)%tauwt(dom(ib)%isp-1:dom(ib)%iep+1,dom(ib)%jsp-1:dom(ib)%jep+1))
        allocate(dom(ib)%tauwb(dom(ib)%isp-1:dom(ib)%iep+1,dom(ib)%jsp-1:dom(ib)%jep+1))
        allocate(dom(ib)%tauwe2(dom(ib)%jsp-1:dom(ib)%jep+1,dom(ib)%ksp-1:dom(ib)%kep+1))
        allocate(dom(ib)%tauww2(dom(ib)%jsp-1:dom(ib)%jep+1,dom(ib)%ksp-1:dom(ib)%kep+1))
        allocate(dom(ib)%tauwn2(dom(ib)%isp-1:dom(ib)%iep+1,dom(ib)%ksp-1:dom(ib)%kep+1))
        allocate(dom(ib)%tauws2(dom(ib)%isp-1:dom(ib)%iep+1,dom(ib)%ksp-1:dom(ib)%kep+1))
        allocate(dom(ib)%tauwt2(dom(ib)%isp-1:dom(ib)%iep+1,dom(ib)%jsp-1:dom(ib)%jep+1))
        allocate(dom(ib)%tauwb2(dom(ib)%isp-1:dom(ib)%iep+1,dom(ib)%jsp-1:dom(ib)%jep+1))
        
        ! Multigrid solver (mgsolver.for)
        if(solver.eq.2) allocate (dom(ib)%faz(ngrd_gl))
        
        ! Ghost cell variable to exchange fluid property between the subdomains:
        ...
        ! Initialiase the grid point location and cell location:
        ...
        ! Error messus if domain has not been set up correctly:
...
!-----------------------------------------------------------------------
!     Initialise the flow field properties at t=0 or t=restart
!#######################################################################
      use vars
      use mpi
      use multidata
...
!===============================================================
        ...
        ELSE ! .NOT. LRESTART (t=0)
          
          qzero=ubulk 								
          qstpn=qzero ; qstpn_y=0.d0 ;  qstpn_z=0.d0 		     
	   ...
	   else if(trim(keyword).eq.'column' .or. trim(keyword).eq.'waves') then
            dom(ib)%u=0.0
     ...
     &               * (dcos(8*PI*dom(ib)%xc(i)) + dcos(20*PI*dom(ib)%xc(i))) 
              ...
              dom(ib)%u(i,j,k)=ubulk*tanh((2*dom(ib)%zc(k)-L0)/v0) + 1E-3 * (sfunc-sfuncy)
...
!NEED TO ADAPT THESE TO LSM
          IF(dom(ib)%bc_west.eq.12 .or. (dom(ib)%bc_west.eq.8 .and. UPROF.eq.12)) THEN	
     ...
     &	                        * (DABS(dom(ib)%zc(k)/(zen-zst)))**(1.d0/7.d0)
	     enddo ; end do ;  end do
          ...
          IF(dom(ib)%bc_west.eq.13 .or. (dom(ib)%bc_west.eq.8 .and. UPROF.eq.13)) THEN		
            do i = 1,tti ; do j = 1,ttj ; do k = 1,ttk
	       if(dom(ib)%yc(j).lt.((yen-yst)/2)) then
                dom(ib)%u(i,j,k) = ubulk*(1.0d0+1.0d0/7.0d0) &
     &	                          * (DABS(2*dom(ib)%yc(j)/(yen-yst)))**(1.d0/7.d0)
     ...
     &	                          * (DABS(2*((yen-yst)-dom(ib)%yc(j))/(yen-yst)))**(1.d0/7.d0)
	     ...
	     enddo ; end do ;  end do
          ...
          IF(dom(ib)%bc_west.eq.14 .or. (dom(ib)%bc_west.eq.8 .and. UPROF.eq.14)) THEN		
            do i = 1,tti ; do j = 1,ttj ; do k = 1,ttk
     ...
     &                         * (DABS(dom(ib)%zc(k)/(zen-zst)))**(1.d0/7.d0)
	     enddo ; end do ;  end do
          ...
          IF(dom(ib)%bc_west.eq.15.or. (dom(ib)%bc_west.eq.8 .and. UPROF.eq.15)) THEN			
            do i = 1,tti ; do j = 1,ttj ; 	 do k = 1,ttk
     ...
     &                         * (1.d0/0.41d0*DLOG(DABS(dom(ib)%zc(k))*0.0187d0*10**6)+5.d0)
	     enddo ; end do ;  end do
          ...
          IF(dom(ib)%bc_west.eq.17.or. (dom(ib)%bc_west.eq.8 .and. UPROF.eq.17)) THEN
            do i = 1,tti ; do j = 1,ttj  ; do k = 1,ttk
