...
!#######################################################################
      ...
      SELECT CASE (order)
        ...
        CASE (1)
          dh = phi_r1smth((xij-Xl)/dx) * phi_r1smth((yij-Yl)/dy) * phi_r1smth((zij-Zl)/dz)
        CASE (2)
          dh = phi_r2smth((xij-Xl)/dx) * phi_r2smth((yij-Yl)/dy) * phi_r2smth((zij-Zl)/dz)
        CASE (3)
          dh = phi_r3smth((xij-Xl)/dx) * phi_r3smth((yij-Yl)/dy) * phi_r3smth((zij-Zl)/dz)
        CASE (4)
          dh = phi_r4smth((xij-Xl)/dx) * phi_r4smth((yij-Yl)/dy) * phi_r4smth((zij-Zl)/dz)
        CASE (5)
          dh = phi_r3((xij-Xl)/dx) * phi_r3((yij-Yl)/dy) * phi_r3((zij-Zl)/dz)
        CASE (6)
          dh = phi_r4((xij-Xl)/dx) * phi_r4((yij-Yl)/dy) * phi_r4((zij-Zl)/dz)
        CASE DEFAULT
          WRITE(6,'(a)')  '===ERROR==='
...
!#######################################################################
      ...
      ELSE IF ((r.ge.-1.5).and.(r.le.0.0)) THEN
        phi_r2smth = 1./4./PI*(PI+2.*sin(PI/4.*(-2.*r+1.))-2.*sin(PI/4.*(-2.*r-1.)))
