...
!           buffer_ppref=dom(ib)%p(ipref,jpref,kpref)+dom(ib)%pp(ipref,jpref,kpref)
!         end if
!       end do

        call MPI_ALLREDUCE(buffer_ppref,ppref,1,MPI_FLT,MPI_SUM,MPI_COMM_WORLD,ierr)

        do ib=1,nbp
          do k=dom(ib)%ksp-1,dom(ib)%kep+1
            do i=dom(ib)%isp-1,dom(ib)%iep+1
              do j=dom(ib)%jsp-1,dom(ib)%jep+1
                dom(ib)%p(i,j,k)=(dom(ib)%p(i,j,k)+(dom(ib)%pp(i,j,k)-ppref))
                dom(ib)%pp(i,j,k)=0.0
              end do
            end do
          end do
 ...
 3000 continue
      ...
      if(myrank.eq.0) then
        ...
        write(6,'(1x,a,i8,a,e13.6,a,i8)')' ntime:',ntime,' rmax:',rmax,' 	  iter',iter
        write(numfile,'(1x,a,i8,a,e13.6,a,i8)') ' ntime:',ntime,' rmax:',rmax,' 	  iter',iter
        write(numfile2,'(i8,f15.6,3e20.6)') ntime,wtimedum,rmax,dt,Mdef
      end if
...
!#######################################################################
      ...
      DO ib=1,nbp
        ...
        dom(ib)%faz(1) = (dom(ib)%iep-dom(ib)%isp+3) * (dom(ib)%jep-dom(ib)%jsp+3) * (dom(ib)%kep-dom(ib)%ksp+3)
        ...
        do glevel=2,ngrd_gl
          ...
          dom(ib)%faz(glevel) = dom(ib)%faz(glevel-1) + mgc_i*mgc_j*mgc_k + 8*(prmgci-2)*(prmgcj-2)*(prmgck-2)
        ...
        do glevel=1,ngrd_gl
          ...
          IF (L_LSM) THEN! .OR. L_LSMBASE) THEN
            do k=2,mgc_k-1 ; do i=2,mgc_i-1 ; do j=2,mgc_j-1
...
!          do k=2,mgc_k-1 ; do i=2,mgc_i-1 ; do j=2,mgc_j-1
	   IF(L_LSM) THEN! .OR. L_LSMBASE) THEN
            ...
            ijk=dom(ib)%faz(glevel)+(k-2)*(mgc_i-2)*(mgc_j-2)+(i-2)*(mgc_j-2)+(j-1)

            if(glevel.eq.1) ijk_lsm=(k-2)*(mgc_i-2)*(mgc_j-2)+(i-2)*(mgc_j-2)+(j-1)
     ...
     &                              + (k-2)*(mgc_i-2)*(mgc_j-2)+(i-2)*(mgc_j-2)+(j-1)
            ...
            if(i.eq.2) then
              ...
              densim12=0.5*(dom(ib)%dens_mg(ijk_lsm)+dom(ib)%dens_mg(ijk_lsm-(mgc_j-2)))
            end if

            if(i.eq.mgc_i-1) then
              densip12=dom(ib)%dens_mg(ijk_lsm)
            else
              densip12=0.5*(dom(ib)%dens_mg(ijk_lsm)+dom(ib)%dens_mg(ijk_lsm+(mgc_j-2)))
            end if

            if(j.eq.2) then
              densjm12=dom(ib)%dens_mg(ijk_lsm)
            else
              densjm12=0.5*(dom(ib)%dens_mg(ijk_lsm)+dom(ib)%dens_mg(ijk_lsm-1))
            end if

            if(j.eq.mgc_j-1) then
              densjp12=dom(ib)%dens_mg(ijk_lsm)
            else
              densjp12=0.5*(dom(ib)%dens_mg(ijk_lsm)+dom(ib)%dens_mg(ijk_lsm+1))
            end if

            if(k.eq.2) then
              denskm12=dom(ib)%dens_mg(ijk_lsm)
            else
              denskm12=0.5*(dom(ib)%dens_mg(ijk_lsm)+dom(ib)%dens_mg(ijk_lsm-(mgc_i-2)*(mgc_j-2)))
            end if

            if(k.eq.mgc_k-1) then
              denskp12=dom(ib)%dens_mg(ijk_lsm)
            else
              denskp12=0.5*(dom(ib)%dens_mg(ijk_lsm)+dom(ib)%dens_mg(ijk_lsm+(mgc_i-2)*(mgc_j-2)))
            end if

            dom(ib)%cof(ijk)=dom(ib)%dy*dom(ib)%dz/dom(ib)%dx/densim12 !aw
            dom(ib)%cof(ijk+1*incr)=dom(ib)%dy*dom(ib)%dz/dom(ib)%dx/densip12 !ae
            dom(ib)%cof(ijk+2*incr)=dom(ib)%dx*dom(ib)%dz/dom(ib)%dy/densjm12 !as
            dom(ib)%cof(ijk+3*incr)=dom(ib)%dx*dom(ib)%dz/dom(ib)%dy/densjp12 !an
            dom(ib)%cof(ijk+4*incr)=dom(ib)%dx*dom(ib)%dy/dom(ib)%dz/denskm12 !ab
            dom(ib)%cof(ijk+5*incr)=dom(ib)%dx*dom(ib)%dy/dom(ib)%dz/denskp12 !at
          end do ; end do ; end do
	   ...
	   END IF
          ...
          if(dom(ib)%iprev.lt.0 .and. dom(ib)%bc_west.ne.5 .and. i.eq.2)        dom(ib)%cof(ijk)=0.d0
          if(dom(ib)%inext.lt.0 .and. dom(ib)%bc_east.ne.5 .and. i.eq.mgc_i-1)  dom(ib)%cof(ijk+1*incr)=0.0
          if(dom(ib)%jprev.lt.0 .and. dom(ib)%bc_south.ne.5 .and. j.eq.2)       dom(ib)%cof(ijk+2*incr)=0.0
          if(dom(ib)%jnext.lt.0 .and. dom(ib)%bc_north.ne.5 .and. j.eq.mgc_j-1) dom(ib)%cof(ijk+3*incr)=0.0
          if(dom(ib)%kprev.lt.0 .and. dom(ib)%bc_bottom.ne.5 .and. k.eq.2)      dom(ib)%cof(ijk+4*incr)=0.0
          if(dom(ib)%knext.lt.0 .and. dom(ib)%bc_top.ne.5 .and. k.eq.mgc_k-1)   dom(ib)%cof(ijk+5*incr)=0.0
...
!#######################################################################
      ...
      DO ib=1,nbp
        ...
        do k=1,mgc_k ; do i=1,mgc_i ; do j=1,mgc_j
	 ...
	 IF(L_LSM)THEN
          do k=2,mgc_k-1 ; do i=2,mgc_i-1 ; do j=2,mgc_j-1
            ijk=dom(ib)%faz(1)+(k-2)*(mgc_i-2)*(mgc_j-2)+(i-2)*(mgc_j-2)+(j-1)+incrp
...
! K-cycle(iprer,ipost) complete at gleve inject correction to finer grid
