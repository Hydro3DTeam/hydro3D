...
!        if (LRESTART.eq..false.) KK1=KK
	...
	do ib=1,nbp
	  ...
	  idfile=600+dom_id(ib)
        ...
        write(b_str,'(I10)') num_output
        strlen=LEN(TRIM(ADJUSTL(b_str)))
        b_str=REPEAT('0',(6-strlen))//TRIM(ADJUSTL(b_str)) ! e.g. "001"

        write(c_str,'(I10)') dom_id(ib)
        strlen=LEN(TRIM(ADJUSTL(c_str)))
        c_str=REPEAT('0',(4-strlen))//TRIM(ADJUSTL(c_str)) ! e.g. "001"

      filename='tecout_'//trim(adjustl(b_str))//'_'//trim(adjustl(c_str))//'.dat'
     ...
     &"k","eps","vis"'
      ...
      WRITE(idfile,*)'zone ','STRANDID=', itime, 'SOLUTIONTIME=', ctime
...
!#######################################################################
      ...
      filename='tecpt_'//trim(adjustl(b_str))//'.dat'

      OPEN (UNIT=95, fiLe=filename)

        WRITE (95,*) 'TITLE = ', '"Lagrangian field"'
        WRITE (95,"(A)") 'VARIABLES = "X","Y","Z","U<sub>Lag<\sub>","V<sub>' &
     &                 //'Lag<\sub>","W<sub>Lag<\sub>","D<sub>Lag<\sub>","Fu","Fv","Fw"'
        WRITE(95,'(a,I8,a,F10.5)')'Zone T="Particles", STRANDID=', itime, ',SOLUTIONTIME=', ctime

        do l=1,np
      	WRITE (95,88) xp_pt(l),yp_pt(l),zp_pt(l) &
     &			,uop_pt(l),vop_pt(l),wop_pt(l) &
...
!	ipux(l) = ip(l) - 1
      ...
      else if(xp_loc(l).le.dom(ib)%xc(ip(l))) then
	ipu(l) = ip(l) - 1
!	ipux(l) = ip(l)
...
!	jpvy(l) = jp(l) - 1
      ...
      else if(yp_loc(l).le.dom(ib)%yc(jp(l))) then
	jpv(l) = jp(l) - 1
!	jpvy(l) = jp(l)
      end if
...
!	kpwz(l) = kp(l) - 1
      ...
      else if(zp_loc(l).le.dom(ib)%zc(kp(l))) then
	kpw(l) = kp(l) - 1
!	kpwz(l) = kp(l)
...
!	endif
      ...
      uoi_pt(l) = 0.d0 ; voi_pt(l) = 0.d0 ; woi_pt(l) = 0.d0

      do i=iballs_u,iballe_u ; do j=jballs_u,jballe_u ; do k=kballs_u,kballe_u
	...
	do i=iballs_v,iballe_v ; do j=jballs_v,jballe_v ; do k=kballs_v,kballe_v
	...
	do i=iballs_w,iballe_w ; do j=jballs_w,jballe_w ; do k=kballs_w,kballe_w
