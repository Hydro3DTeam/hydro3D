...
!#######################################################################
      ...
      buffer_umax=umax
      call MPI_ALLREDUCE (buffer_umax,umax,1,MPI_FLT,MPI_MAX,MPI_COMM_WORLD,ierr )
      ...
      buffer_vmax=vmax
      call MPI_ALLREDUCE (buffer_vmax,vmax,1,MPI_FLT,MPI_MAX,MPI_COMM_WORLD,ierr)
      ...
      buffer_wmax=wmax
      call MPI_ALLREDUCE (buffer_wmax,wmax,1,MPI_FLT,MPI_MAX,MPI_COMM_WORLD,ierr)
     ...
     &                 +2.0*dom(ib)%vis(i,j,k)*(1.0/dxx+1.0/dyy+1.0/dzz)+small)
      ...
      if (L_LSM)  then
        do ib=1,nbp
          dtvisc=max(mul/densl,mug/densg)*(2.0/(dxx)+2.0/(dyy)+2.0/(dzz))
          Cu=1./((umax/dom(ib)%dx+dtvisc)+sqrt((umax/dom(ib)%dx+dtvisc)**2+4.*abs(grx)/dom(ib)%dx))
          Cv=1./((vmax/dom(ib)%dy+dtvisc)+sqrt((vmax/dom(ib)%dy+dtvisc)**2+4.*abs(gry)/dom(ib)%dy))
          Cw=1./((wmax/dom(ib)%dz+dtvisc)+sqrt((wmax/dom(ib)%dz+dtvisc)**2+4.*abs(grz)/dom(ib)%dz))
          dt = 2.*safety_factor*min(Cu,Cv,Cw)
         end do
      end if

      buffer_dtmax=dt
      call MPI_ALLREDUCE (buffer_dtmax,dt1,1,MPI_FLT,MPI_MIN,MPI_COMM_WORLD,ierr )
